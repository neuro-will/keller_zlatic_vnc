""" Tests for state, decision or reporting dependence in DFF, referencing each behavior to a single condition.
...
pdf_save_path = '/Users/bishopw/Desktop/results/decision_dependence_quiet_ref_by_segment.pdf'

# ======================================================================================================================
# Here we specify a path to a folder we can use for saving temporary files (images)
temp_folder = '/Users/bishopw/Desktop/single_dep_temp'
...
for cell_type, cell_ids in cell_types:
    ...
    # Read in the neural activity for the specified cell type & specify the correct look up for segment codes
    if cell_type == 'a00c':
    ...
    for cut_off_time in cut_off_times:
        for manip_type in manip_types:
            ...
            # Determine which behaviors are present before and after the manipulation, removing behaviors we will test
            # for if they are not present in enough subjects
            ...
            one_hot_data_ref, one_hot_vars_ref = one_hot_from_table(data, beh_before=before_behs_ref,
            ...
            if (n_before_behs > 0) and (n_after_behs > 0) and (not np.min(v) < .001):
